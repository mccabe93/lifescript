...
//		{$ast = new NeighborsStmt($ID.text, $coordexp.ast, $STRING.text);}
	...
	|	'Properties' ':'
			...
			('Dimensions' '=' dims=coordexp)? // otherwise 540,540
			...
			('Start' '=' '{' 
				...
				{startConditions.addAST(new CellStmt($celltype.text, $cellcoords.ast));}
				')')* '}' )?
			('Generations' '=' gens=exp)? // otherwise infinite
			('Interval' '=' intervals=exp)? // otherwise 0.3 seconds
			('Pausable' '=' pause=exp)? // defaults true
			('Steppable' '=' sb=exp)? // defaults false 
		{System.out.println("new properties = " + $title.text + ", " + $dims.ast+ ", " + $gens.ast + 
					", " + $intervals.ast + ", "+ $pause.ast + ", " + $sb.ast);
			$ast = new PropertiesStmt($title.text, $dims.ast, startConditions, $gens.ast, 
						$intervals.ast, $pause.ast, $sb.ast);}
	;
